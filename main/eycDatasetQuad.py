...
class EycDataset(Dataset):
    ...
    def __init__(self, zip_path="eyc-data_50.tar.gz", train=False, train_size=800):
        """
    ...
    def __getitem__(self, idx):
        ...
        if probability < 0:
            while True:
                negative2_tuple = random.choice(self.dataset_pre.imgs)
                if negative2_tuple[1] != anchor_tuple[1] and negative2_tuple[1] != negative_tuple[1]:
                    break
        else:
            while True:
                negative2_tuple = random.choice(self.dataset_post.imgs)
                if negative2_tuple[1] != anchor_tuple[1] and negative2_tuple[1] != negative_tuple[1]:
                    break
        ...
        negative2 = Image.open(negative2_tuple[0])
        ...
        negative2 = negative2.convert("L")

        anchor = transform(anchor)
        positive = transform(positive)
        negative = transform(negative)
        negative2 = transform(negative2)
